...
## 9) Примечания по домофону СОЮЗ (из инструкции производителя)

Полезные ограничения и параметры, которые стоит учитывать при настройке:
- SIP: устройство поддерживает до 2 SIP аккаунтов и до 5 SIP‑панелей на квартиру.
- DTMF‑открытие: параметр для открытия двери во время SIP‑вызова настраивается в интерфейсе устройства.
- RTSP: поток доступен по `rtsp://root:123456@<ip>:8554` (измените пароль на реальный).
- WEB/UI: интерфейс управления по умолчанию на `http://<ip>:8088`.
- API: базовый URL по умолчанию `http://<ip>:8081`, дефолтные логин/пароль `api/123456`.
